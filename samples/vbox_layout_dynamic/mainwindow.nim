...
proc addNewButton(self: MainWindow) =
  echo "addNewButton"
  self.buttonCount += 1
  var newButton = QPushButton.create("Button " & $self.buttonCount, self.window)
  
  # Insert the new button before the "Add Button" button
  let addButtonIndex = self.hboxLayout.indexOf(self.addButton)
  QBoxLayout(self.hboxLayout).insertWidget(addButtonIndex, newButton)
  newButton.owned = false
...
proc newMainWindow*(): MainWindow =
  ...
  result.button1 = QPushButton.create("Button 1", result.window)
  result.button2 = QPushButton.create("Button 2", result.window)
  result.button3 = QPushButton.create("Button 3", result.window)
  result.addButton = QPushButton.create("Add Button", result.window)
  result.label = QLabel.create("QhboxLayout arranges widgets vertically from top to bottom", result.window)
  ...
  QBoxLayout(result.hboxLayout).addWidget(result.button1)
  QBoxLayout(result.hboxLayout).addWidget(result.button2)
  QBoxLayout(result.hboxLayout).addWidget(result.button3)
  QBoxLayout(result.hboxLayout).addWidget(result.addButton)
  QBoxLayout(result.hboxLayout).addWidget(result.label)
